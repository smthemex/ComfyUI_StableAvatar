...
class WanI2VTalkingInferenceLongPipeline(DiffusionPipeline):
    ...
    def __call__(
        ...
        #print(masked_video_latents_input.shape,mask_input.shape)#torch.Size([3, 16, 31, 64, 64]) torch.Size([3, 4, 31, 64, 64])
                ...
                while index_end <= infer_length: #31
                    ...
                    if index_start != 0 and i != 0:
                        ...
                        for iii in range(legal_compressed_frames_num):
                            ...
                            pred_latents[:, :, index_start:index_end] = latents[:, :, :index_end-index_start]
        ...
        #print(latents.shape) #torch.Size([1, 16, 207, 60, 104])
        ...
        if output_type == "numpy":
            ...
            std_inv = torch.tensor([
                1.0/2.8184, 1.0/1.4541, 1.0/2.3275, 1.0/2.6558, 1.0/1.2196, 1.0/1.7708, 1.0/2.6052, 1.0/2.0743,
                1.0/3.2687, 1.0/2.1526, 1.0/2.8652, 1.0/1.5579, 1.0/1.6382, 1.0/1.1253, 1.0/2.8251, 1.0/1.9160
            ], dtype=torch.float32)

            video=apply_scale_to_latent(latents, mean, std_inv)

        # Offload all models
        self.maybe_free_model_hooks()
        if not return_dict and output_type != "latent":
            video = torch.from_numpy(video)
        return WanI2VPipelineTalkingInferenceLongOutput(videos=video)

def apply_scale_to_latent(latent, mean, std_inv):
    """
    在使用comfy的VAE解码前，将scale应用到latent数据
    latent: 来自StableAvatar的latent数据 [B, C, T, H, W]
    mean: StableAvatar VAE的mean参数 [C]
